...
if ($email && $password) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($password, $hashedPassword)) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO user_sessions (user_id, session_number, token, expiration_time) VALUES (?, ?, ?, ?)");
            $insertStmt->bind_param("iiss", $userId, $sessionNumber, $token, $expirationDatetime);
            $insertStmt->execute();

            if ($insertStmt->affected_rows > 0) {
                echo json_encode(["status" => "success", "userId" => $userId, "token" => $token]);
            } else {
